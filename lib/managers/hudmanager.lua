...
function HUDManager:init_finalize()
	...
	if not self:exists(PlayerBase.PLAYER_INFO_HUD) then
...
function HUDManager:set_mugshot_location(id, location_id)
	...
	for i, data in ipairs(self._hud.mugshots) do
		if data.id == id then
			local s = utf8.to_upper(managers.localization:text(location_id))
...
function HUDManager:_set_mugshot_state(id, icon_data, text)
	...
	data.state_text:set_text(utf8.to_upper(text))
...
function HUDManager:add_mugshot_equipment(id, equipment)
	...
	for i, data in ipairs(self._hud.mugshots) do
		if data.id == id then
			...
			self:add_teammate_special_equipment(managers.criminals:character_data_by_name(data.character_name_id).panel_id, {
				id = equipment,
...
function HUDManager:_layout_mugshot_equipment(data)
	local icon_size = 34
	for i, e_data in ipairs(data.equipment) do
		local align = i == 1 and data.mask or data.equipment[i - 1].icon
...
function HUDManager:set_weapon_name(name_id)
	...
	hud.weapon_name:set_text(utf8.to_upper(managers.localization:text(name_id)))
end
