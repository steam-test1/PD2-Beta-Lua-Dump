...
function SavefileManager:storage_changed()
	...
	if storage_device_selected then
		print("LOADING SEQUENCE STARTED", Application:time())
		self._loading_sequence = true
		self._loading_save_games = {
			[self.SETTING_SLOT] = true,
			[self.PROGRESS_SLOT] = true
		}
...
function SavefileManager:check_space_required()
	...
	local task = NewSave:all_slots(param_map)
	local task_handler = SavefileTaskHandler:new(task, self.CHECK_SPACE_REQUIRED_TASK_TYPE, callback(self, self, "_space_required_slot_detected_callback"), callback(self, self, "_space_required_slot_detected_done_callback"))
	self:_add_task_handler(task_handler)
end
function SavefileManager:_space_required_slot_detected_callback(slot)
	print("SavefileManager:_space_required_slot_detected_callback()", slot)
	self._space_required_list = self._space_required_list or {}
	table.insert(self._space_required_list, slot)
end
function SavefileManager:_space_required_slot_detected_done_callback()
	print("SavefileManager:_space_required_slot_detected_done_callback()", inspect(self._space_required_list))
	if not self._space_required_list then
		print(" HAD NO SAVE GAMES")
		if SystemInfo:platform() == Idstring("PS3") then
			self._savegame_hdd_space_required = 2 * self.RESERVED_BYTES / 1024
		end
	else
		print(" #self._space_required_list", #self._space_required_list)
		if SystemInfo:platform() == Idstring("PS3") then
			self._savegame_hdd_space_required = (2 - #self._space_required_list) * self.RESERVED_BYTES / 1024
		end
	end
end
...
function SavefileManager:update(t, dt)
	...
	if self._loading_sequence and not next(self._loading_save_games) then
		self._loading_sequence = nil
		print("LOADING SEQUENCE COMPLETE", Application:time())
...
function SavefileManager:break_loading_sequence()
	...
	self._try_again = nil
	self._has_meta_list = nil
	self._loading_save_games = nil
	self._loading_sequence = nil
...
function SavefileManager:_meta_data_loaded_callback(save_data)
	...
	print("savefile_manager", "[SavefileManager] Meta data loaded. Slot: " .. tostring(slot) .. ", Text: \"" .. tostring(save_data:subtitle()) .. "\", Status: " .. tostring(SaveData.status_to_string(status)) .. " (" .. tostring(status) .. ")")
...
function SavefileManager:_save(slot, cache_only, save_system)
	...
	if cache_only then
		...
		if is_setting_slot and managers.user.STORE_SETTINGS_ON_PROFILE then
			...
			local unique_id = is_progress_slot and "progress" or nil
			local task = NewSave:save(save_data, param_map)
			local task_handler = SavefileTaskHandler:new(task, self.SAVE_TASK_TYPE, callback(self, self, "_save_callback"), nil, unique_id)
			self:_add_task_handler(task_handler)
		end
	end
end
function SavefileManager:_save_platform_setting_map_callback(success)
	print("SavefileManager:_save_platform_setting_map_callback", success)
...
function SavefileManager:_load_done(slot, cache_only, wrong_user)
	...
	elseif self._loading_save_games then
		self._loading_save_games[slot] = nil
	end
end
...
function SavefileManager:_add_task_handler(task_handler)
	if task_handler:unique_id() and self._task_handler_list and #self._task_handler_list > 0 then
		for i, task in ipairs(self._task_handler_list) do
			if task:unique_id() and task:unique_id() == task_handler:unique_id() then
				self._task_handler_list[i] = task_handler
				return
			end
		end
	end
...
function SavefileManager:_ask_load_backup(reason, dialog_at_init, load_params)
	...
	function yes_button.callback_func()
		...
		self._loading_save_games[self.PROGRESS_SLOT] = nil
	end
	function no_button.callback_func()
		self._backup_data = nil
		self._loading_save_games[self.PROGRESS_SLOT] = nil
...
function SavefileTaskHandler:init(task, task_type, callback_func, done_callback_func, unique_id)
	self._task = task
...
function SavefileTaskHandler:unique_id()
...
function LoadPlatformSettingMapTask:unique_id()
	return nil
end
...
function SavePlatformSettingMapTask:unique_id()
	return nil
end
