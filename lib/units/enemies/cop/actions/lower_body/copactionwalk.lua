	...
	if move_dir and not self._expired then
		...
		if self._chk_stop_dis and not self._common_data.char_tweak.no_run_stop then
			...
			if end_dis < self._chk_stop_dis then
				...
				if math.abs(fwd_dot) > math.abs(r_dot) then
					if fwd_dot > 0 then
						stop_anim_side = "fwd"
					else
						stop_anim_side = "bwd"
	...
	if not reservation_failed then
		...
		elseif new_c_index ~= self._curve_path_index or nav_advanced then
			...
			if future_pos then
				...
				if self._haste ~= "run" and turn_dot > -0.7 and turn_dot < 0.7 and not self._attention_pos and future_dis_flat > 80 and self._common_data.stance.name == "ntl" then
				...
				if vis_state < 3 and self._end_of_curved_path and not self._NO_RUN_STOP and not self._no_walk and self._haste ~= "walk" and not (mvec3_dis(c_path[new_c_index + 1], new_pos) < 120) and not not self._ext_anim.run then
					self._chk_stop_dis = 210
				end
				self._walk_turn = nil
			end
