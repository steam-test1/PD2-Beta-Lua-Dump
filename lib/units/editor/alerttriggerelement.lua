...
function AlertTriggerElement:_build_panel(panel, panel_sizer)
	...
	for i, o in ipairs(alert_type_table) do
		...
		check:connect("EVT_COMMAND_CHECKBOX_CLICKED", callback(self, self, "on_alert_type_checkbox_changed"), {ctrlr = check, name = o})
	...
	for i, o in ipairs(opt) do
		...
		check:connect("EVT_COMMAND_CHECKBOX_CLICKED", callback(self, self, "on_filter_checkbox_changed"), {ctrlr = check, name = o})
